...
public class BankTool {
	private static final int IDCARD_MIN_LENGTH = 16;
	private static final int IDCARD_MAX_LENGTH = 19;

	private static final String[] PROVINCE = "山东,江苏,上海,浙江,安徽,福建,江西,广东,广西,海南,河南,湖南,湖北,北京,天津,河北,山西,内蒙古,宁夏,青海,陕西,甘肃,新疆,四川,贵州,云南,重庆,西藏,辽宁,吉林,黑龙江,香港,澳门,台湾".split(",");
	...
	private static String[] bankName = {
		"邮政储蓄银行·绿卡通", "邮政储蓄银行·绿卡银联标准卡", "邮政储蓄银行·绿卡银联标准卡", "邮政储蓄银行·绿卡专用卡", "邮政储蓄银行·绿卡银联标准卡",
		"邮政储蓄银行·绿卡(银联卡)", "邮政储蓄银行·绿卡VIP卡", "邮政储蓄银行·银联标准卡", "邮政储蓄银行·中职学生资助卡", "邮政储蓄银行·IC绿卡通VIP卡",
		"邮政储蓄银行·IC绿卡通", "邮政储蓄银行·IC联名卡", "邮政储蓄银行·IC预付费卡", "邮政储蓄银行·绿卡银联标准卡", "邮政储蓄银行·绿卡通",
		...
		"玉溪市商业银行·红塔卡", "建设银行·借记卡"
	};

	public static String getNameOfBank(String idCard) {
		String name;
		if (idCard == null || idCard.length() < IDCARD_MIN_LENGTH || idCard.length() > IDCARD_MAX_LENGTH) {
			return "";
		}
		...
		name = getName(cardbin8);
		return name;
	}

	private static String getName(String cardbin) {
		int index = -1;
	...
	public static Bank getRegion(String cardNo, String branch) throws UnsupportedException {
		String bankName = BankTool.getNameOfBank(cardNo);
		if (bankName != null && bankName.contains(GAP)) {
			bankName = bankName.substring(0, bankName.indexOf("·"));
		}
		String bankCode = PayRegion.getRegion(bankName);
		if (bankCode == null) {
			throw new UnsupportedException("您的开户银行暂不支持兑换，请更换其它开户银行卡");
		}
		...
		for (String s : PROVINCE) {
			if (branch.contains(s)) {
				province = s;
				branch = branch.replace(s, "").replace("省", "").replace("自治区", "");
				break;
			}
		}
